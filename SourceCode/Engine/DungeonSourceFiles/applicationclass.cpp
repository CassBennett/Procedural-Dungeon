...
{
	// Set all variables to default values or to null to set up the class
	m_Input = 0;
	m_Direct3D = 0;
	m_Camera = 0;
	m_MapCamera = 0;
...
bool ApplicationClass::Initialize(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	float mapCameraX = -5000.0f;
	float mapCameraY = 55.0f;
	float mapCameraZ = 0.0f;
	...
	result = m_Dungeon->Initialise(m_Direct3D->GetDevice(), m_Direct3D->GetDeviceContext(), &hwnd);
	if (!result)
	{
		if (m_Dungeon->DungeonFailed)// The size of the map available for the dungeon
		{
			MessageBox(hwnd, L"Could not generate the dungeon because the specified grid is too small.\nTo adjust the grid size, change the value of DUNGEON_GRID_X and DUNGEON_GRID_Y in DungeonClass.h\nOr reduce the dungeon depth value DUNGEON_DEPTH.", L"Error", MB_OK);
		}
		else
		{
	...
	for (auto DungeonPiece = m_Dungeon->GetDungeonPieces()->begin(); DungeonPiece != m_Dungeon->GetDungeonPieces()->end(); ++DungeonPiece)
	{
...
void ApplicationClass::ToggleCollision(bool keyDown)
{
	...
	if ((m_ButtonTimer <= 0) && (keyDown))
	{
		...
		else if (!m_CollisionOn)
		{
			...
			D3DXVECTOR3 PlayerLastPos = m_Dungeon->GetPlayerLastPos();
...
bool ApplicationClass::RenderDungeon(D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, CameraClass* currentCamera)
{
	...
	for (auto CeilingPiece = m_Dungeon->GetCeilingPieces()->begin(); CeilingPiece != m_Dungeon->GetCeilingPieces()->end(); ++CeilingPiece)
	{
		// Push mesh data onto gfx hardware
		(*CeilingPiece)->Render();
...
bool ApplicationClass::RenderMap(D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix)
{
	...
	m_Direct3D->GetWorldMatrix(worldMatrix);
	D3DXMatrixTranslation(&translationMatrix, -5000.0f, 0.0f, 0.0f);
	...
	for (auto DoorPiece = m_Dungeon->GetDoorPieces()->begin(); DoorPiece != m_Dungeon->GetDoorPieces()->end(); ++DoorPiece)
	{
		// Push mesh data onto gfx hardware
		(*DoorPiece)->Render();
	...
	D3DXMatrixTranslation(&translationMatrix, m_Dungeon->GetPlayerPosition().x - 5000, m_Dungeon->GetPlayerPosition().y, m_Dungeon->GetPlayerPosition().z);
	...
	m_Dungeon->GetPlayerMesh()->Render(m_Direct3D->GetDeviceContext());
	...
	result = m_TextureShader->Render(m_Direct3D->GetDeviceContext(), m_Dungeon->GetPlayerMesh()->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, m_Dungeon->GetPlayerMesh()->GetTexture());
	if (!result)
	{
		return false;
	}
