...
bool DungeonClass::Initialise(ID3D11Device* Device, ID3D11DeviceContext* DeviceContext)
{
	bool result;
	m_Device = Device;
	...
	result = m_EntrancePiece->Initialise(m_Device, m_DeviceContext, D3DXVECTOR2(20.0f, 20.0f), 2.0f, 2.0f, 4);
	if (!result)
	{
		return false;
	}
	...
	result = CeilingPiece->Initialise(m_Device, m_DeviceContext, D3DXVECTOR2(20.0f, 20.0f), 2.0f, 2.0f, 4);
	if (!result)
	{
		return false;
	}
	...
	CheckGrid(2.0f, 2.0f, D3DXVECTOR2(20.0f, 20.0f));
	...
	// from that new piece.
	PlaceNextPiece(0, m_EntrancePiece, RIGHT);
	PlaceNextPiece(0, m_EntrancePiece, UP);
	PlaceNextPiece(0, m_EntrancePiece, LEFT);
	PlaceNextPiece(0, m_EntrancePiece, DOWN);
...
void DungeonClass::PlaceNextPiece(int Depth, DungeonPieceClass *Piece, int DoorNumber)
{
...
void DungeonClass::Shutdown()
{
	...
	// Release the entrance piece
	if (m_EntrancePiece)
	{
		delete m_EntrancePiece;
		m_EntrancePiece = 0;
	}
}
...
bool DungeonClass::CheckGrid(float Width, float Height, D3DXVECTOR2 Center)
{
	...
	int Start_x = Center_X - HalfWidthLeft;
	int Start_y = Center_Y - HalfHeight;
	int End_x = Center_X + HalfWidthRight;
	int End_y = Center_Y + HalfHeight;

	// If the starting position is outside of the dungeon grid return false straight away
	if ((Start_x > DUNGEON_GRID_X) || (Start_y > DUNGEON_GRID_Y))
	{
		return false;
	}
...
void DungeonClass::SetChances(int depth)
{
	...
	else
	{
		...
		// Change the rooms limit to 2 so that less rooms are available to be created than at the start of the dungeon
		m_RoomsLimit = 3.0f;
	}
...
void DungeonClass::FillGridWithWall(MeshClass *Wall)
{
	...
	if (Wall->m_MeshAlongXAxis)
	{
		...
		start = Wall->m_StartPos.x * 10;
		end = Wall->m_EndPos.x * 10;
		// Get the fixed z position of the wall so it can be used for the grid
		int z_pos = Wall->m_StartPos.z * 10;
	...
	else if (Wall->m_MeshAlongZAxis)
	{
		...
		start = Wall->m_StartPos.z * 10;
		end = Wall->m_EndPos.z * 10;
		// Get the fixed x position of the wall so it can be used for the grid
		int z_pos = Wall->m_StartPos.z * 10;
		int x_pos = Wall->m_StartPos.x * 10;
		for (int i = start; i <= end; i++)
		{
			// Fill the grid so that the dungeon knows there is a collision here
...
bool DungeonClass::CanMovePlayer(D3DXVECTOR3 lookat, int direction)
{
	...
	// Get the next player position into the grid position by multiplying by 10
	int xpos = m_NextPlayerPosition.x * 10;
	int ypos = m_NextPlayerPosition.z * 10;
